...
// 06-1C — program.status = 'draft' (수업안은 만들었지만 아직 확정 전)
...
export const ClassDetailsLessonTabDraftScreen = ({ navigation, route }: any) => {
  ...
  const handleContinueConfirm = () => {
    navigation?.navigate('LessonPlanConfirm', { classId, className });
  };
    ...
    <ScreenLayout title={className} showBackButton scrollable={false}>
      ...
      <View className="pb-lg">
        ...
        <Card variant="default" className="px-md py-md mb-lg">
          ...
          <Text className="text-caption text-ink-secondary">수업안을 만들었지만 아직 확정 전이에요</Text>
        </Card>
        <Button label="이어서 확정하기" onPress={handleContinueConfirm} />
      </View>
