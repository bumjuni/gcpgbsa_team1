...
export const ClassDetailsMemberTabAddingScreen = ({ navigation, route }: any) => {
  ...
  const handleSubmit = () => {
    ...
    navigation?.navigate('ClassDetailsMemberTab', { classId, className });
  };

  return (
    <ScreenLayout title={className} showBackButton>
      <View className="flex-row border-b border-hairline mb-md">
        {TABS.map((tab) => {
          const active = tab === '명단';
          return (
      ...
      <View className="pb-xl">
        ...
        <Card className="p-md">
          ...
          <Button label="추가하기" variant="secondary" onPress={handleSubmit} />
        </Card>
      </View>
    </ScreenLayout>
  );
