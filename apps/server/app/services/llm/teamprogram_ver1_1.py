# teamprogram_ver1_1.py — 팀(강사) 모드, teamprogram_ver1.py 계승
# 근거: programlogic.md (§1 공통 코어, §2 팀 설계)
#
# ver1 대비 변경 (강사가 4개 예시(team_program_examples.md) 실행 결과를 검토한
# 뒤 준 피드백 6가지 반영):
...
def generate_curriculum(request_body: dict) -> dict:
    ...
    # ★ 강사 피드백: EN2 이상 interval_set 존도 Main-Set에 드릴을 "추가로" 섞을
    # 수 있게 해달라는 요청 — 존별로 허용 강도만 다른 별도 후보 목록을 만든다.
    # main_drill_list(need_main_drills)와는 완전히 분리된 변수라 content_type
    # =='drill' 카테고리(A/D 등) 쪽 로직·프롬프트 변수는 전혀 건드리지 않는다.
    interval_extra_drill_sections = []
    for cat in categories:
        zp = zone_plan[cat]
        zone_key = zp.get("zone")
        allowed = ZONE_MAIN_EXTRA_DRILL_INTENSITIES.get(zone_key)
        if zp["content_type"] == "interval_set" and allowed:
            cat_drills = [d for d in drills if d.get("intensity") in allowed]
            if cat_drills:
